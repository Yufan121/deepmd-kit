...
class DescrptSeACopy (DescrptSe):
    ...
    def _pass_filter(self, 
        ...
        if not self.type_one_side and type_embedding is None:
            ...
            # get z from xyz
            z_coords_atoms = coord[:, 2:natoms[0]*3:3]
            z_coords_atoms = tf.reshape(z_coords_atoms, [-1, 1])
            ...
            dists_to_inter = tf.reshape(dists_to_inter, [natoms[0], 1])
            print("dists_to_inter.shape" + str(dists_to_inter.shape))
            tf.summary.histogram('dists_to_inter', dists_to_inter)
            ...
            # tf.print(dists_to_inter)
            layer = tf.concat([layer, dists_to_inter], 1, name="concat_dist_inter") # layers of shape (natoms[1], M1*M2 + 1)
    ...
    def get_dist_to_interface(self, # Yufan
        ...
        if axis_interface == 'z':
            ...
            box_1d = z_box
            # else : # type embedding net is enabled
            #     pass
            
        elif axis_interface == 'y':
            pass
        elif axis_interface == 'x': 
            pass
    ...
    def _filter(
        ...
        outputs_size = [1] + self.filter_neuron # add 1 to the start of sel
        ...
        with tf.variable_scope(name, reuse=reuse):
            ...
            result = tf.matmul(xyz_scatter_1, xyz_scatter_2, transpose_a = True) # matmul to get the embedding output
            # natom x (outputs_size x outputs_size_2)
